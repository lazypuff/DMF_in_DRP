...
def manifold_learning(Train_IC, P, Q, K, SimD, SimC, landa, miu):
    ...
    index_row = np.arange(0, Train_IC.shape[0])
    random.shuffle(index_row)
    validnum_row = Train_IC.shape[0] // 10
    test_index_row = index_row[0:validnum_row]
    validPosition = all_position[np.isin(all_position[:, 0], test_index_row)]
