...
def runapp(response_file, simC_name, simD_name, fold_infos, percent, miu, landa, CV_num, repetition, output_name):
    ...
    #reading IC50 file
    R = np.loadtxt(response_file, dtype=float, delimiter=",", skiprows=1, usecols=range(1,154))
    R_info = pd.read_csv(response_file, index_col=0)
        
    # reading similarity matrices
    simD = np.loadtxt(simD_name, dtype=float, delimiter=",", skiprows=1, usecols=range(1,154))
    simC = np.loadtxt(simC_name, dtype=float, delimiter=",", skiprows=1, usecols=range(1,920))
