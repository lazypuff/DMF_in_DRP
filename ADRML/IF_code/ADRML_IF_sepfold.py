...
edited by XIAO Meisheng AUG 15 2023, the loose splitting(mask combination) strategy 10-CV using Identical Folds
...
def runapp(response_file, simC_name, simD_name, fold_infos, percent, miu, landa, CV_num, repetition, output_name):
    ...
    R = pd.read_csv(response_file, index_col=0)
    R = R.to_numpy()
    R = np.nan_to_num(R)
        
    # reading similarity matrices
    simD = pd.read_csv(simD_name, index_col=0)
    simD = simD.to_numpy()
    simC = pd.read_csv(simC_name, index_col=0)
    simC = simC.to_numpy()
    ...
    ## manual CV, input fold number here
