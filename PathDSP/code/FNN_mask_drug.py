...
modified by Meisheng Xiao, Jul 6 2023, modified the code into mask cell splitting cross validation
"""
...
if __name__ == "__main__":
    ...
    df1 = pd.read_csv(args.input_path, header=0, sep="\t")
    drug_list = pd.unique(df1['drug'])
    ...
    for i in range(kFold):
        ...
        drug_test_set = []
        n_fold = i + 1
        print('Fold={:}/{:}'.format(n_fold, kFold))
        if n_fold == 10:
            drug_test_set = drug_list[s * i:len(drug_list)]
        else:
            drug_test_set = drug_list[s * i:(i + 1) * s]

        drug_train_set = list(set(drug_list) - set(drug_test_set))
        drug_train_set_final, drug_valid_set = skms.train_test_split(drug_train_set,
                                                                    test_size=0.1, random_state=args.seed_int)
        train_df = df1[df1['drug'].isin(drug_train_set_final)]
        valid_df = df1[df1['drug'].isin(drug_valid_set)]
        test_df = df1[df1['drug'].isin(drug_test_set)]
        train_df_desired = train_df.set_index(['drug', 'cell'])
        valid_df_desired = valid_df.set_index(['drug', 'cell'])
        test_df_desired = test_df.set_index(['drug', 'cell'])
        Xtrain_arr = train_df_desired.iloc[:, 0:-1].values
        Xtest_arr = test_df_desired.iloc[:, 0:-1].values
        Xvalid_arr = valid_df_desired.iloc[:, 0:-1].values
        ytrain_arr = train_df_desired.iloc[:, -1].values
        ytest_arr = test_df_desired.iloc[:, -1].values
        yvalid_arr = valid_df_desired.iloc[:, -1].values
        print('    train={:}, valid={:}, test={:}'.format(Xtrain_arr.shape, Xvalid_arr.shape, Xtest_arr.shape))
        # prepare dataframe for output
        ytest_df = test_df_desired.iloc[:, -1].to_frame()
