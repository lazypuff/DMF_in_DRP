...
if __name__ == "__main__":
    ...
    for i in range(kFold):
        ...
        if n_fold == 10:
            cell_test_set = cell_list[s * i:len(cell_list)]
        else:
            cell_test_set = cell_list[s * i:(i + 1) * s]

        cell_train_set = list(set(cell_list) - set(cell_test_set))
        cell_train_set_final, cell_valid_set = skms.train_test_split(cell_train_set,
                                                                    test_size=0.1, random_state=args.seed_int)
        train_df = df1[df1['cell'].isin(cell_train_set_final)]
        valid_df = df1[df1['cell'].isin(cell_valid_set)]
        test_df = df1[df1['cell'].isin(cell_test_set)]
