...
Editted by Meisheng Xiao Aug 1, 2023
"""
...
def parse_parameter():
    ...
    parser.add_argument("-o", "--output_path",
                        required = True,
                        help = "output path")
    parser.add_argument("-f", "--fold_info_path",
                        required = True,
                        help = "the fold info path to read in the specific fold_info for CV")
...
if __name__ == "__main__":
    ...
    # load CV fold information
    fold_info = pd.read_csv(args.fold_info_path)
    # set parameters
    ...
    for i in range(kFold):
        ...
        fold_train = fold_info[(fold_info['fold number'] == n_fold) & (fold_info['indicator'] == 'train')]
        fold_valid = fold_info[(fold_info['fold number'] == n_fold) & (fold_info['indicator'] == 'valid')]
        # Get the 'pair information' for fold 1, train
        fold_train_pairs = fold_train['pair information'].tolist()
        fold_valid_pairs = fold_valid['pair information'].tolist()
        # Convert the 'pair information' to a MultiIndex
        fold_train_multiindex = pd.MultiIndex.from_tuples([tuple(pair.split('/')) for pair in fold_train_pairs])
        fold_valid_multiindex = pd.MultiIndex.from_tuples([tuple(pair.split('/')) for pair in fold_valid_pairs])
        # Find the intersection of MultiIndex
        train_index = df.index.get_indexer(fold_train_multiindex)
        test_index = df.index.get_indexer(fold_valid_multiindex)
        # Filter
