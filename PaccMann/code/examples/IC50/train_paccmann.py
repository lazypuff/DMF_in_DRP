...
def main(
    ...
    for epoch in range(params["epochs"]):
        ...
        predictions = np.stack([p.cpu() for preds in predictions for p in preds])
        #predictions2 = np.stack([p.cpu() for preds in predictions for p in preds])
        #predictions3 = np.array(predictions)
        labels = np.stack([l.cpu() for label in labels for l in label])
        #labels2 = np.array(labels)

        test_rmse_a = np.sqrt(np.mean((predictions.astype(float) - labels.astype(float)) ** 2))
        test_loss_a = test_loss / len(test_loader)
        test_pearson_a = pearsonr(torch.Tensor(predictions[:,0]), torch.Tensor(labels[:,0]))
