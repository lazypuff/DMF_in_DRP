...
Drugsim_fig_mt = readmatrix('simD_ogdsc_1024b.csv');
...
fold_info = readmatrix('srmf_valid_info_mask_comb_og.csv');
...
for j = 1:10
    ...
    index = (1:size(train_df,1)*size(train_df,2));
    index = randsample(index, size(train_df,1)*size(train_df,2));
    results = [];
...
writematrix(results_met,'results_oldgdsc_1024b_mask_comb.csv');
writematrix(predMatOut,'predMat_oldgdsc_1024b_mask_comb.csv');