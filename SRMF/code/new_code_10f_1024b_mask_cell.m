...
fold_info = readmatrix('srmf_valid_info_mask_cell_og.csv');
...
for j = 1:10
    ...
    % index for training use
    index = (1:size(resp_mat,2));
    index = randsample(index, size(resp_mat,2));
    results = [];
...
writematrix(results_met,'results_oldgdsc_1024b_mask_cell.csv');
writematrix(predMatOut,'predMat_oldgdsc_1024b_mask_cell.csv');